...
public class LoginHandler extends HttpServlet {
	...
	protected boolean getmailyn(String account, String password){
		MemVO memVO = memsev.findByAccount(account);
		if (memVO != null) {
			if (memVO.getMemgetmailyn().equals("Y")) {
				return true;
			} else {
				return false;
			}
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		if (!allowUser(account, password)) { // 【帳號 , 密碼無效時】
			req.setAttribute("errorLogin", "帳號或密碼無效!");
			RequestDispatcher failureView = req
					.getRequestDispatcher("/front/mem/login.jsp");
			failureView.forward(req, res);
		}else if(!getmailyn(account, password)){
			req.setAttribute("errorLogin", "信箱未通過認證!");
			RequestDispatcher failureView = req
					.getRequestDispatcher("/front/mem/login.jsp");
			failureView.forward(req, res);
